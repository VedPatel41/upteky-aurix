 ...
 * Features subtle staggered reveal and responsive card transitions.
 */
export default function ProcessFunnel({ stages = [], delay = 160 }) {
  return (
    <div
      style={{ animationDelay: `${delay}ms` }}
      className="aurix-card bg-white border border-[#E4E8EE] rounded-[6px] p-5 mb-8 animate-card-reveal"
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4 pb-3 border-b border-[#F0F3F7]">
        ...
        <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-[4px] bg-[#FEF6E9] border border-[#FCE1B6] text-[#B8781B] text-[12px] font-medium self-start sm:self-auto transition-transform hover:scale-[1.01]">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 sm:gap-1.5 items-center">
        {stages.map((stage, idx) => {
          ...
          const stageDelay = `${delay + idx * 50}ms`;

          return (
            <div key={stage.id} className="relative flex items-center">
              {/* Stage Card */}
              <div
                style={{ animationDelay: stageDelay }}
                className={`w-full p-3 rounded-[4px] border transition-all duration-200 animate-card-reveal hover:-translate-y-0.5 ${
                  isWeakest
                    ? "bg-[#FFFBF5] border-[#E8A33D] shadow-2xs ring-1 ring-[#E8A33D]/25"
                    : "bg-[#FAFBFC] border-[#E4E8EE] hover:bg-[#F4F7FB] hover:border-[#D1D9E4]"
                }`}
              >
                <div className="flex items-center justify-between mb-1">
                  ...
                  {isWeakest && (
                    <span className="text-[10px] font-bold px-1.5 py-0.2 rounded bg-[#E8A33D] text-white animate-checkmark-pop">
                      Drop-off
                    </span>
                  )}
                </div>
