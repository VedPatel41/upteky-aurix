...
function useCountUp(endValue, duration = 900, isCurrency = false, decimals = 0) {
  ...
  if (isCurrency) {
    // Format Indian Rupee currency: e.g. ₹2,40,000
...
const CustomChartTooltip = ({ active, payload }) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-[#0E1B2B] text-white px-3 py-2 rounded-[4px] shadow-md text-[12px]">
 ...
 * Renders the 3 animated impact metrics, Recharts Before/After comparison, and visible disclaimer.
 */
export default function ImpactSummary({ impactData }) {
  ...
  return (
    <div className="w-full space-y-8">
      {/* 3 Large Impact Metric Tiles */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        {/* Metric 1: Hours Saved */}
        <div className="aurix-card bg-white border border-[#E4E8EE] rounded-[6px] p-6 shadow-xs flex flex-col justify-between transition-all hover:border-[#CBD5E1]">
          ...
          <div>
            <div className="text-[40px] font-semibold text-[#0E1B2B] tabular-nums tracking-tight leading-none my-1">
              {hoursSaved}
            </div>
        ...
        <div className="aurix-card bg-white border border-[#E4E8EE] rounded-[6px] p-6 shadow-xs flex flex-col justify-between transition-all hover:border-[#CBD5E1]">
          ...
          <div>
            <div className="text-[36px] lg:text-[40px] font-semibold text-[#0E1B2B] tabular-nums tracking-tight leading-none my-1">
              {revenueOpportunity}
            </div>
            <p className="text-[14px] font-medium text-[#5A6B7B] mt-1.5">
              Revenue opportunity
      ...
      {/* Main Visual: Before vs After Bar Chart Card */}
      <div className="aurix-card bg-white border border-[#E4E8EE] rounded-[6px] p-6 shadow-xs">
        ...
        <div className="mt-4 p-3 bg-[#F4F8F6] border border-[#D5EADF] rounded-[4px] flex items-center justify-between text-[13px]">
      ...
      {/* Mandatory Impact Disclaimer (Section 24) */}
      <div className="p-3.5 bg-[#F7F8FA] border border-[#E4E8EE] rounded-[6px] flex items-start sm:items-center gap-2.5">
